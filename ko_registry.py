...
ko_registry.py
--------------
Uradni šifrant katastrskih obcin Republike Slovenije (šifra -> ime),
povzet po Prilogi 1 Pravilnika o dolocitvi obmocij in imen katastrskih
obcin (Uradni list RS, št. 139/2021). Uporablja se za dinamicno
iskanje/prikaz imena katastrske obcine v zavihku 'E-ZEMLJIŠKA KNJIGA':
ko uporabnik vpiše šifro, se ime izpiše sproti; ime obcine pa lahko
uporabnik tudi neposredno poišce, kar samodejno izpolni šifro.
"""

from __future__ import annotations

# šifra (str) -> ime katastrske obcine
KO_BY_CODE: dict[str, str] = {
    "1": "Hodoš",
    "2": "Suhi Vrh",
    "3": "Dolenci",
    "4": "Budinci",
...
# (šifra, ime) urejeno po imenu - za iskanje/spustni seznam po imenu
KO_LIST_BY_NAME: list[tuple[str, str]] = sorted(KO_BY_CODE.items(), key=lambda kv: kv[1])


def get_ko_name(ko_code: str) -> str:
    """Vrne ime katastrske obcine za dano šifro (dinamicno iskanje po
    celotnem uradnem šifrantu ~2700 KO), ali opozorilo, ce šifra ni
    najdena - v tem primeru iskanje v eSodstvu vseeno lahko deluje
    (šifrant se obcasno posodablja).
    """
    if not ko_code:
        return ""
    return KO_BY_CODE.get(ko_code.strip(), "(šifra ni najdena v šifrantu - iskanje v eSodstvu vseeno poskusite)")


def search_ko_by_name(query: str, limit: int = 25) -> list[tuple[str, str]]:
    """Vrne seznam (šifra, ime) katastrskih obcin, katerih ime vsebuje
    iskani niz (brez razlikovanja velikih/malih crk) - za dinamicno
    iskanje po imenu."""
    q = (query or "").strip().lower()
    if not q:
        return []
    return [(c, n) for c, n in KO_LIST_BY_NAME if q in n.lower()][:limit]
